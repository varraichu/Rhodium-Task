...
const Car_Form = () => {
    ...
    try {
      ...
      const uploadedUrls = await Promise.all(
        files.map(async (file) => {
          const { url } = await fetch("https://rhodium-backend-c7497f7bdb81.herokuapp.com/s3Url").then(res => res.json());
          console.log(url);
          await fetch(url, {
            method: 'PUT',
            headers: {
  ...
  const uploadPost = async () => {
    try {
      ...
      const response = await fetch('https://rhodium-backend-c7497f7bdb81.herokuapp.com/cars/post', {
        method: 'POST',
        headers: {
